...
async function fetchAndProcessData() {
  try {
    ...
    // Fetch both sheets
    const gaertenCSV = await fetchCSV(
      `https://docs.google.com/spreadsheets/d/${SHEET_ID}/export?format=csv&gid=${GAERTEN_SHEET_GID}`,
    )
    const datenCSV = await fetchCSV(
      `https://docs.google.com/spreadsheets/d/${SHEET_ID}/export?format=csv&gid=${DATEN_SHEET_GID}`,
    )
    ...
    const datenIndices = findColumnIndices(datenHeaders, [
      'GARTEN_ID',
      'TAG',
      'VON',
      'BIS',
      ...
      if (invalidDates.length > 0) {
        ...
        invalidDates.forEach((dateEntry, index) => {
          if (!dateEntry.TAG || dateEntry.TAG.trim() === '') {
            ...
            errors.push(
              `date ${index + 1}: TAG format invalid (expected DD.MM.YYYY, got "${dateEntry.TAG}")`,
            )
          }

          if (dateEntry.VON && !/^\d{1,2}:\d{2}$/.test(dateEntry.VON)) {
            errors.push(
              `date ${index + 1}: VON format invalid (expected HH:MM, got "${dateEntry.VON}")`,
            )
          }

          if (dateEntry.BIS && !/^\d{1,2}:\d{2}$/.test(dateEntry.BIS)) {
            errors.push(
              `date ${index + 1}: BIS format invalid (expected HH:MM, got "${dateEntry.BIS}")`,
            )
          }
        })
      }
    ...
    // Summary statistics
    const gardensWithErrors = processedGardens.filter(
      (garden) => garden.errors && garden.errors.length > 0,
    )
